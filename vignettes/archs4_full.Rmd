...
```{r}
rowStats=computeRowStatsFBM(archs4FBM, chunk_size=1000)
...
message(paste("New filtered dataset dims: ", nrow(archs4FBMfiltered), " x ", ncol(archs4FBMfiltered)))
gene_symbols_thin=gene_symbols_thin[filterResult$kept_rows]
message(paste("Number of genes kept: ", dim(length(gene_symbols_thin))))
...
Do the z-scoring
```{r}
zscoreFBM(archs4FBMfiltered, rowStats = rowStats, chunk_size=1000)
```
