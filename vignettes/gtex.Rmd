...
```{r}
output_nb_path <- here("output/gtex")
...
exprs_path  <- here("data/gtex/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_tpm.gct.gz")
output_file <- here("data/gtex/bulk-gex_v8_rna-seq_GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_tpm.rds")
...
```{r}
gtex=readRDS(here("data/gtex/bulk-gex_v8_rna-seq_GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_tpm.rds"))
```
...
```{r}
k = 200
gtexFBM.svd=big_randomSVD(gtexFBMfiltered, k = k)
save(gtexFBM.svd, file.path(output_nb_path, "svd.rds"))
```
...
if (!file.exists(decomp_file)) {
  gtex.sdres <- simpleDecomp(gtexFBMfiltered, k = k, svdres = gtexFBM.svd)
