...
saveRDS(
  ...
  file.path(output_dir, "genes_meta.rds")
)
```
...
# zscoreFBM(filterResult$fbm_filtered, rowStats = rowStats, chunk_size=config$GENERAL$CHUNK_SIZE)

options(bigstatsr.check.parallel.blas = FALSE)
blas_nproc <- getOption("default.nproc.blas")
options(default.nproc.blas = NULL)

zscoreFBM_opt(fbm_obj_filtered, rowStats = rowStats, ncores=config$GENERAL$N_CORES)
