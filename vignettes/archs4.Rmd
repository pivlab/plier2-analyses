...
Pick 10% of samples at random

```{r}
set.seed(42)
keep_cols <- sort(sample.int(n_samples, size = floor(0.1 * n_samples)))
n_keep    <- length(keep_cols)
```
...
fbm_file_sub   <- file.path(output_nb_path, "FBMarchs4_10pct")
archs4FBM_sub  <- FBM(
  nrow        = n_genes,
  ncol        = n_keep,
  backingfile = fbm_file_sub,
  create_bk   = TRUE
)
```
...
block_size <- 100
blocks     <- split(keep_cols, ceiling(seq_along(keep_cols)/block_size))
...
for (b in seq_along(blocks)) {
  ...
  col_idx <- match(cols, keep_cols)
  archs4FBM_sub[, col_idx] <- mat
}
...
```{r}
archs4.plier2=PLIERv2(archs4FBMfiltered, matchedPaths, sdres = archs4.sdres, Chat = Chat, doCrossval = T, multiplier = 3, max.U.updates = 3, max.iter = 350, k = k)
```
