...
```{r}
set.seed(123);iisample=sample(1:ncol(gtexFBMfiltered),2000)
meta=list()
...
Create the FBM
```{r}
fbm_file <- "FBMgtex"  # Specify the file name for the gtexFBM
#if already created
...
```
Run PLIER, setting `max.iter` low for demonstration
```{r}
gtex.plier=PLIERv2(gtexFBMfiltered, matchedPaths, gtex.sdres, Chat = Chat,doCrossval = T, multiplier = 3, max.U.updates = 3, max.iter = 20)
```
